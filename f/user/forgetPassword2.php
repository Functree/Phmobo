...
if (isset($_POST["email"]) && isset($_POST["imageCode"])) {
    $email = $_POST["email"];
    ...
    if (strtolower($imageCode) == $_SESSION["imageCode"]) {
        ...
        if ($document != null) {
            ...
            $resetPasswordUrl = FUNCTREE_WEB_ROOT."User/resetPassword?userId=$userId&token=$token";
            UserUtil::c_one( [ 'id' => $userId ], [ 'token' => $token, 'tokenTime' => $now ] );
            $to = $email;
            $subject = "来自" . FUNCTREE_WEB_NAME . "的重置密码邮件";
            $body = "<html><meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"><body>尊敬的".$userName."：<br><br>您正在重置您在" . FUNCTREE_WEB_NAME . "的用户密码，请点击下面链接完成重置密码操作：<br><a href=\"$resetPasswordUrl\">$resetPasswordUrl</a><br><br>请在1小时内完成操作，谢谢！<br><br>温情提示：请勿回复此邮件，此邮件为系统自动发送。</body></html>";
            if (UserUtil::sendMail(FUNCTREE_WEB_NAME, $to, $subject, $body)) {//发送Email
            ?>
